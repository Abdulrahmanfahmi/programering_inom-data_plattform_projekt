...
async def fetch_riksmote(client, rm):
    if rm is None:
        ...
        url = f"https://data.riksdagen.se/anforandelista/?sz=500&rm={rm.replace('/', '%2F')}&utformat=json"
...
async def main():
    while True:
        print("Hämtar anföranden — live + historisk...")
        async with httpx.AsyncClient(timeout=30.0, headers=HEADERS) as client:
            for rm in RIKSMOTEN:
                try:
                    raw = await fetch_riksmote(client, rm)
                    cleaned = clean_anforanden(raw)
                    send_to_kafka("anforanden_topic", cleaned)
                    label = "live" if rm is None else rm
                    print(f" {label}: {len(cleaned)} anföranden skickade")
                except Exception as e:
                    label = "live" if rm is None else rm
                    print(f"  {label}: {e}")
        print("Anföranden klar! Väntar 30 min...")
